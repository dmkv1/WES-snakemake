...
20. ~~**Add somalier — genotype-based sample-swap and sex QC**~~ ✅ — fixed: new `somalier_extract` (per-sample, off the final recalibrated BAM) / `somalier_ped` (per-run PED encoding samplesheet `gender`) / `somalier_relate` (per-run gather) chain in `workflow/rules/somalier.smk`, wired into `rule all` and into `multiqc`'s inputs so MultiQC's native somalier module picks up the report. Runs via conda (`somalier=0.3.2`), not a container — the `brentp/somalier:v0.3.2`/`v0.3.2-1` Docker images both ship a broken `/bin/bash` (readline symbol relocation failure), so bioconda was used instead, matching how this repo already handles other small CLI tools (bcftools, mosdepth, fastqc). Report-only, no DAG gating: mismatches surface in the HTML/MultiQC report for human review. Verified end-to-end on `P005` (CTRL/PT pair): both samples' inferred sex matches samplesheet `gender` (m → male), and CTRL/PT relatedness is 0.997 with 1.000 concordance, confirming correct tumor/normal pairing. (Samplesheet later migrated from `Chr_sex: XX/XY` to `gender: f/m`; this entry updated to match.)
...
25. ~~**PureCN is not wired into the DAG**~~ ✅ — fixed: `purecn.smk` is now `include`d in `Snakefile`, gated to paired tumor/normal runs (PDX included, tumor-only excluded — no matched-normal het-SNP track to estimate BAF from). `purecn_run` now passes `--normaldb`/`--mapping-bias-file` (from the new per-kit PureCN NormalDB built in `WES-PON-smk`, see `purecn_normaldb_wes_pon_smk.md`) and `--snp-blacklist` (UCSC simple repeats, hg38), none of which were wired before. The original draft's `cnvkit_call_with_purity`/`cnvkit_plots_calibrated` rules — a parallel, never-actually-consumed `.purecn_calibrated.cns` output — are deleted; instead a new `resolve_purity_source` rule is the single decision point for purity/ploidy (PureCN when `params.cnv.purity_source: "purecn"` in config and not `Flagged`, else the samplesheet constant), and both `cnvkit_call` and `combine_results.R` read that same sidecar CSV so they can no longer independently disagree on which purity value was used (previously `get_purity()` was called separately in both places — harmless only because both happened to read the same samplesheet scalar). Default config value is `"samplesheet"` — zero behavior change until explicitly flipped. PureCN itself always runs for eligible samples regardless of the toggle, and its raw Purity/Ploidy/Flagged status is now surfaced in a new `Sample_QC` sheet in each sample's results xlsx and in a new cohort-wide `results/combined/combined_qc.tsv`, enabling the "PureCN vs samplesheet" comparison the toggle exists for. A genuine `PureCN.R` crash (bad path, env issue) hard-fails the DAG like every other rule — it does not silently fall back to samplesheet purity; only a *successful* run that reports `Flagged=TRUE` does. Run end-to-end on `P005` (CTRL/PT/PDX) against the production, sex-split NormalDB (`panel_of_normals.purecn.{normaldb,mapping_bias}_{m,f}`, built by `WES-PON-smk`): `purecn_run` completes, and the output CSV's column names (`Purity`/`Ploidy`/`Flagged`) match what `resolve_purity_source` assumed from the vignette — no parsing hard-fail. With the toggle still on `"samplesheet"`, `.call.cns`/xlsx outputs are byte-identical to the pre-#25 baseline, confirming the sidecar plumbing doesn't alter behavior until explicitly flipped. Both `P005_PT` and `P005_PDX_VFND2` came back `Flagged=TRUE` ("NON-ABERRANT; LOW PURITY; POOR GOF") — PureCN's own log shows it correctly detecting a near-flat copy-number profile rather than misbehaving, and `resolve_purity_source` correctly fell back to samplesheet purity for both. `params.cnv.purity_source` remains `"samplesheet"`; flipping it is still a deliberate future step once a CN-aberrant sample is available to validate against.
