...
26. **Conda envs under-pinned — reproducibility risk, materialized once already.** `envs/pysam.yaml` (added for #22) pinned only `pysam=0.24.0` with no Python version — the solver picked `python-3.14.6` **free-threaded** (`cp314t`), which forces the GIL back on at import (`pysam.libchtslib` isn't declared thread-safe) with a `RuntimeWarning` on every job. Harmless this time (pysam still imports and runs correctly), but it means this env is not reproducible across re-solves — a future `conda env create` on the same `pysam.yaml` could land on a different Python minor/major or build variant with no warning at all, silently changing behavior.
    - **Fix `pysam.yaml` now:** pin `python=3.12` (or another non-free-threaded build) alongside `pysam=0.24.0`.
    - **Audit the rest of `workflow/envs/*.yaml` for the same class of gap** — checked current resolved installs (`.snakemake/conda/*/conda-meta/*.json`) against each yaml:
        - `envs/r_vcf.yaml` — worst offender. Only `r-base=4.3` is pinned (major.minor, no patch/build); `bioconductor-variantannotation`, `r-tidyverse`, `r-openxlsx` are completely unpinned. Currently-resolved, verified-working versions: `r-base=4.3.3`, `bioconductor-variantannotation=1.48.1`, `r-tidyverse=2.0.0`, `r-openxlsx=4.2.8` — pin all four to these.
        - `envs/purecn.yaml` — `r-optparse` unpinned (moot while dead code per #25, but pin if/when wired in).
        - `envs/manta.yaml`, `envs/bcftools.yaml`, `envs/bwamem.yaml`, `envs/fastp.yaml`, `envs/qc.yaml`, `envs/somalier.yaml`, `envs/xengsort.yaml`, `envs/annotsv.yaml` — every direct dependency already has a version pin, no action needed there.
    - **Deeper, unresolved gap (applies even to the fully-pinned envs above):** none of these pin a conda **build string** (e.g. `bcftools=1.21=h8b25389_0`), only the version — a re-solve can still swap the build (different linked `htslib`/`openssl`/etc.) with no version-number signal that anything changed. This is the same concern `portability-todo.md`'s section 2 preamble already names as the reason to move these rules to `container:` instead of `conda:` — full fix is that migration, not further pinning. Treat the pins above as a cheap stopgap for the conda envs that remain, not a substitute for section 2.
    - **Effort:** low (the two yaml edits); the build-string/container migration is already tracked separately in `portability-todo.md`.
